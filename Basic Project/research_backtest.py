#!/usr/bin/env python3
"""Independent next-open research simulator for SatoshiFlow robustness checks.
...
def write_outputs(
    ...
    summary = [
        "SATOSHIFLOW INDEPENDENT RESEARCH ROBUSTNESS BACKTEST",
        "Engine: stricter next-open mark-to-market simulator",
...
def main(argv: list[str] | None = None) -> int:
    # Import the official signal implementation instead of maintaining a second
    # strategy definition. This file changes only the execution/fee model.
    from main import (
        DEFAULT_CONFIG as OFFICIAL_CONFIG,
        assert_signal_shift,
        dataset_hash as official_dataset_hash,
        load_dataset as official_load_dataset,
        process_data as official_process_data,
        resolve_dataset as official_resolve_dataset,
        run_lookahead_check,
        strat as official_strat,
    )

    args = build_parser().parse_args(argv)
    dataset_path = official_resolve_dataset(args.data)
    results_dir = (
        Path(args.results_dir).resolve()
        if args.results_dir
        else Path(__file__).resolve().parent / "results" / "research"
    )
    ...
    if json.dumps(metrics, sort_keys=True, allow_nan=False) != json.dumps(
        ...
        raise AssertionError("research metrics changed between identical runs")
    pd.testing.assert_frame_equal(trades, repeat_trades, check_exact=True)
    pd.testing.assert_frame_equal(equity, repeat_equity, check_exact=True)
    ...
    metrics.update(
        {
            "engine": "independent_next_open_mark_to_market",
            ...
            "parameters": asdict(OFFICIAL_CONFIG),
            "lookahead_check": True,
            "next_bar_execution_check": True,
